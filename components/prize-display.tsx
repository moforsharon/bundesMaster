...
export default function PrizeDisplay({ participantId, challenge }: PrizeDisplayProps) {
  ...
  // Update the getWinnerMessage function to handle ranks 1-10 and 11+
  const getWinnerMessage = (position: number) => {
    if (locale === "fr") {
      if (position <= 10) {
        return "Félicitations! Vous avez une chance de gagner le grand prix. Nous vous contacterons bientôt."
      } else {
        return "Désolé, vous n'êtes pas qualifié pour le prix. Restez à l'affût du prochain défi!"
      }
    } else {
      if (position <= 10) {
        return "Great! You stand a chance of winning the grand prize. We will contact you soon."
      } else {
        return "Oops! You did not qualify for the prize. Please watch out for the next challenge!"
      }
    }
  }
